...
const onBlurBirthdate = (inputEl) => {
  emit('blur')
  focused.value = false

  if (typeof inputEl === 'string' && birthdate.value[inputEl]) {
    const el = document.getElementById(`${props.id}-${inputEl}`)
    if (el) {
      if (birthdate.value[inputEl] >= parseInt(el.max)) {
        birthdate.value[inputEl] = parseInt(el.max)
      } else if (birthdate.value[inputEl] <= parseInt(el.min)) {
        birthdate.value[inputEl] = parseInt(el.min)
      }
    }
  }
}
