<template>
    <header class="bg-white z-50 border-b border-gray-100">
      <nav class="mx-auto flex max-w-7xl items-center justify-between p-6 lg:px-8" aria-label="Global">
        <div class="flex lg:flex-1">
          <a href="#" class="-m-1.5 p-1.5">
            <span class="sr-only">Your Company</span>
            <NuxtLink to="/"><img class="h-8 w-auto" src="@/assets/svg/logo.svg" alt="" /></NuxtLink>
          </a>
        </div>
        ...
        <PopoverGroup class="hidden lg:flex lg:gap-x-12">
          ...
          <NuxtLink href="/templates" class="text-sm/6 font-semibold text-gray-900">Templates</NuxtLink>
  
          <NuxtLink href="/pricing" class="text-sm/6 font-semibold text-gray-900">Pricing</NuxtLink>
          <NuxtLink href="/faq" class="text-sm/6 font-semibold text-gray-900">Faq</NuxtLink>
          ...
          <Popover class="relative">
            ...
            <transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0 translate-y-1" enter-to-class="translate-y-0" leave-active-class="transition ease-in duration-150" leave-from-class="translate-y-0" leave-to-class="opacity-0 translate-y-1">
              <PopoverPanel class="absolute left-1/2 z-10 mt-3 w-96 -translate-x-1/2 rounded-3xl bg-white p-4 shadow-lg outline-1 outline-gray-900/5">
                <div v-for="item in resources" :key="item.name" class="relative rounded-lg p-4 hover:bg-gray-50">
        ...
        <div class="hidden lg:flex lg:flex-1 lg:justify-end">
          <NuxtLink href="/login" class="text-sm/6 font-semibold text-gray-900"><HbButton size="sm" variant="link">Log in</HbButton></NuxtLink>
          <NuxtLink href="/register" class="text-sm/6 font-semibold text-gray-900"><HbButton size="sm" variant="primary">Sign up</HbButton></NuxtLink>
        </div>
      </nav>
  ...
  <script setup>
  import HbButton from './UI/HbButton.vue'
  import { ref } from 'vue'
  ...
  const resources= [
    { name: 'Videos', href: '#', description: 'Watch our latest videos and learn more about our company values and mission to empower others' },
    { name: 'Blog', href: '#', description: 'Read our latest blog posts and get perspectives from our team' },
    {
      name: 'Support',
      href: '#',
      description: 'Get in touch with our dedicated support team or reach out on our community forums',
    },
    { name: 'Manuals', href: '#', description: 'Read our latest announcements and get perspectives from our team' },
  ]
